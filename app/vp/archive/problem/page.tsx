...
function inlineMarkdown(value: string, keyPrefix = "inline"): ReactNode[] {
  ...
  for (const match of value.matchAll(tokenPattern)) {
    ...
    if (token.startsWith("$$") || token.startsWith("$")) {
      ...
      result.push(<span className={display ? "archive-math-display" : "archive-math-inline"} key={key} dangerouslySetInnerHTML={{ __html: katex.renderToString(expression, { throwOnError: false, strict: "ignore", output: "htmlAndMathml" }) }} />);
    } else if (token.startsWith("`")) {
      result.push(<code key={key}>{token.slice(1, -1)}</code>);
    } else if (token.startsWith("**")) {
      result.push(<strong key={key}>{inlineMarkdown(token.slice(2, -2), `${key}-strong`)}</strong>);
    } else {
      const link = token.match(/^\[([^\]]+)]\((https?:\/\/[^)\s]+)\)$/);
      result.push(link ? <a href={link[2]} target="_blank" rel="noreferrer" key={key}>{link[1]}</a> : token);
    }
...
function StatementBlock({ block }: { block: ArchiveStatementBlock }) {
  if (block.kind === "bullets") return <ul>{block.items.map((item, index) => <li key={index}>{inlineMarkdown(item, `bullet-${index}`)}</li>)}</ul>;
  if (block.kind === "code") return <pre className={`archive-statement-code${block.language ? ` language-${block.language}` : ""}`}><code>{block.code}</code></pre>;
...
function ArchiveStatementView({ statement, language }: { statement: ArchiveExtractedStatement; language: "english" | "chinese" }) {
  ...
  const isPdf = statement.source.kind === "official-pdf-extract";
  const primarySourceUrl = statement.source.englishPdfUrl || statement.source.sourceUrl || "";
  const samples = statement.samples?.length ? statement.samples : statement.sample ? [statement.sample] : [];
  return <article className="archive-statement-panel">
    <header className="archive-statement-toolbar">
      <div><b>结构化题面</b><span>{isPdf ? "正文、样例与图片已从官方题册提取" : "原题、中文、公式与样例已整理到本站"}</span></div>
      <nav>
        {primarySourceUrl ? <a href={primarySourceUrl} target="_blank" rel="noreferrer">{isPdf ? "下载原始 PDF ↓" : "查看原始来源 ↗"}</a> : null}
    ...
    <div className="statement-body full-statement archive-extracted-statement">
      <div className="statement-facts">
        ...
        <span><b>{isOfficialChinese ? `官方中文题册${pages ? ` ${pages} 页` : ""}` : language === "chinese" ? "本站校对中文" : statement.source.sourceLabel || (isPdf ? "Official PDF" : "Original mirror")}</b>{language === "chinese" ? "题面来源" : "Statement source"}</span>
      </div>
      ...
      {samples.length ? <section className="archive-statement-section archive-sample-section">
        <h2>{language === "chinese" ? "样例" : "Examples"}</h2>
        {samples.map((sample, index) => <div className={`samples statement-sample${sample.output ? "" : " transcript"}`} key={`${sample.input}-${index}`}>
          <div>
            <header><b>{sample.mode === "transcript" ? (language === "chinese" ? `交互记录${samples.length > 1 ? ` ${index + 1}` : ""}` : `Transcript${samples.length > 1 ? ` ${index + 1}` : ""}`) : (language === "chinese" ? `样例输入${samples.length > 1 ? ` ${index + 1}` : ""}` : `Input${samples.length > 1 ? ` ${index + 1}` : ""}`)}</b><CopySampleButton value={sample.input} label="复制输入" /></header>
            <pre>{sample.input}</pre>
          </div>
          {sample.output ? <div>
            <header><b>{language === "chinese" ? `样例输出${samples.length > 1 ? ` ${index + 1}` : ""}` : `Output${samples.length > 1 ? ` ${index + 1}` : ""}`}</b><CopySampleButton value={sample.output} label="复制输出" /></header>
            <pre>{sample.output}</pre>
          </div> : null}
        </div>)}
      </section> : null}
      <div className="source-callout"><b>{isOfficialChinese ? "官方中文题面" : language === "chinese" ? "中文题面" : "Original statement"}</b><p>{isOfficialChinese ? "正文、约束、样例与图片均提取自官方题册。" : language === "chinese" ? "公式、代码、样例和图片与原题逐段对应。" : isPdf ? "Text, constraints, samples, and figures were extracted from the official PDF." : "The original structured statement is stored locally for direct reading."}</p></div>
    </div>
  </article>;
}
...
function ArchiveSubmitDialog({ contest, currentSlot, slots, onClose }: { contest: ArchiveContest; currentSlot: string; slots: string[]; onClose: () => void }) {
  ...
  async function submit() {
    ...
    if (!problem) return setError("这道题暂时没有可用的评测入口");
    const selectedLanguage = SUBMIT_LANGUAGES.find((item) => item.value === languageValue) || SUBMIT_LANGUAGES[0];
    const judgeName = problem.judge === "luogu" ? "洛谷" : "Universal Cup / QOJ";
    ...
    if (extensionReady) {
      ...
      await recordPlatformSubmission({
        requestId, judge: problem.judge, problemCode: `${contest.id}-${selectedSlot}`, problemTitle: title,
        problemHref: `/vp/archive/problem?contest=${encodeURIComponent(contest.id)}&slot=${encodeURIComponent(selectedSlot)}`,
        contestId: problem.contestId, problemIndex: selectedSlot, language: selectedLanguage.label,
        status: "queued", message: `正在连接${judgeName}`,
  ...
  const currentJudgeName = currentProblem?.judge === "luogu" ? "洛谷" : "Universal Cup / QOJ";
  return <div className="archive-submit-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="archive-submit-dialog" role="dialog" aria-modal="true" aria-labelledby="archive-submit-title">
      <header><div><small>提交到{currentJudgeName}</small><h2 id="archive-submit-title">提交代码</h2></div><button type="button" aria-label="关闭提交窗口" onClick={onClose}>×</button></header>
...
export default function ArchiveProblemPage() {
  ...
  useEffect(() => {
    ...
    setStatement(null);
    setStatementMessage("");
    if (!problemId || !contestName) return;
    const refresh = async () => {
      try {
