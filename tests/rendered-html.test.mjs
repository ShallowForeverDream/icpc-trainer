...
test("ships the domestic API, SQLite persistence, cached statements, OCR, and local translation deployment", async () => {
  ...
  assert.match(statements, /ARCHIVE_TRANSLATION_VERSION = 5/);
  assert.match(statements, /assessOfficialChineseArchive/);
  assert.match(statements, /TRANSLATION_VERSION = 23/);
  assert.match(statements, /translationQualityIssues/);
...
test("uses one validated training profile across dashboard, catalog, submissions, and VP", async () => {
  ...
  assert.match(account, /syncTrainerPreferences/);
  for (const source of [home, catalog, submissions, vp]) {
    assert.match(source, /readTrainerPreferences/);
    assert.match(source, /syncTrainerPreferences/);
  }
});
