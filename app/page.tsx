...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(browserApiUrl("/codeforces/recommendations?handle=ShallowDream2&min=1200&max=1800&limit=12"), { cache: "no-store" }).then((response) => response.json()),
    ]).then(([submissionData, recommendationData]) => {
  ...
  const activity = useMemo(() => Object.fromEntries(days.map((key) => [key, Math.max(manualActivity[key] ?? 0, remoteActivity[key] ?? 0)])), [days, manualActivity, remoteActivity]);
  ...
  return <AppShell active="训练台">
    <section className="training-dashboard-head">
      ...
      <div><a className="button button-primary" href={`/problem?recommended=1`}><Icon name="play" /> 开始今日训练</a><a className="button button-ghost" href="/vp"><Icon name="trophy" /> 创建 VP</a></div>
    </section>
    ...
    <section className="training-overview-grid">
      ...
      <article className="recent-completed-card panel">
        <div className="panel-head"><div><h2>最近完成</h2><p>{syncing ? "正在同步 Codeforces…" : `累计记录 ${totalActivity} 题`}</p></div><a href="/submissions">全部记录 →</a></div>
        <div className="recent-solve-list">{recentAccepted.length ? recentAccepted.map((item) => <a href={`/problem/${item.code.replace("CF ", "")}`} key={item.id}><Icon name="check" /><span><b>{item.code} · {item.title}</b><small>{new Date(item.createdAt).toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}</small></span></a>) : <p className="dashboard-empty">还没有同步到近期 AC，完成一道题后会显示在这里。</p>}</div>
      </article>
    </section>

    <section className="dashboard-main-grid">
      <article className="panel dashboard-recommendations">
        <div className="panel-head"><div><h2>为你推荐</h2><p>已排除 ShallowDream2 做过的题，按近期训练与目标难度排序。</p></div><button className="text-button" onClick={() => setBatch((value) => value + 1)}><Icon name="shuffle" /> 换一组</button></div>
        <div className="problem-list">{visibleRecommendations.map((problem, index) => <div className="recommended-problem" key={problem.code}><ProblemRow problem={problem} index={index + 1} /><span>{problem.reason}</span></div>)}</div>
        <a className="panel-footer-link" href="/problem?recommended=1">调整标签和目标区间 <span>→</span></a>
      </article>
      <article className="panel compact-heatmap-card">
        ...
        <a className="dashboard-vp-cta" href="/vp"><span><b>创建一场 VP</b><small>支持多人实时榜单与多场组合</small></span><strong>→</strong></a>
      </article>
    </section>
  </AppShell>;
}
