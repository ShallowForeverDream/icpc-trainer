...
export default function AccountPage() {
  ...
  return <AppShell active="账号">
    ...
    <div className="account-grid">
      <form className="panel account-form" onSubmit={savePreferences}><h2>训练设置</h2><label>Codeforces Handle<input value={codeforcesHandle} onChange={(event) => setCodeforcesHandle(event.target.value)} autoComplete="off" maxLength={24} required /></label><label>每日目标<input type="number" min="1" max="20" value={dailyGoal} onChange={(event) => setDailyGoal(Number(event.target.value))} required /></label>{preferenceMessage ? <p className={preferenceMessage === "训练偏好已保存" ? "form-success" : "form-error"}>{preferenceMessage}</p> : null}<button className="button button-primary">保存</button></form>
